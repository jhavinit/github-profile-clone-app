const SiteFooter = () => {
    const year = new Date().getFullYear();

    return (
        <footer className="border-border mt-10 py-6">
            <div className="container mx-auto px-4 flex justify-center">

                <nav className="flex flex-wrap items-center gap-4 text-xs text-muted-foreground">

                    {/* Clickable Logo */}
                    <a
                        href="https://github.com"
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center"
                    >
                        <svg
                            height="18"
                            width="18"
                            viewBox="0 0 16 16"
                            className="fill-muted-foreground hover:fill-primary transition-colors"
                        >
                            <path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 
  7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.67-.52-.01-.53.62-.01 
  1.07.58 1.23.82.71 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 
  0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.22 
  2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 
  2 .27 1.53-1.04 2.2-.82 2.2-.82.44 
  1.1.16 1.92.08 2.12.51.56.82 
  1.28.82 2.15 0 3.07-1.87 
  3.75-3.65 3.95.29.25.54.73.54 1.48 
  0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.013 
  8.013 0 0 0 16 8c0-4.42-3.58-8-8-8z"></path>
                        </svg>
                    </a>

                    {/* Copyright */}
                    <span className="text-muted-foreground">© {year} GitHub, Inc.</span>

                    {/* Links */}
                    <a href="#" className="hover:text-primary">Terms</a>
                    <a href="#" className="hover:text-primary">Privacy</a>
                    <a href="#" className="hover:text-primary">Security</a>
                    <a href="#" className="hover:text-primary">Status</a>
                    <a href="#" className="hover:text-primary">Community</a>
                    <a href="#" className="hover:text-primary">Docs</a>
                    <a href="#" className="hover:text-primary">Contact</a>
                    <a href="#" className="hover:text-primary">Manage cookies</a>
                    <a href="#" className="hover:text-primary">Do not share my personal information</a>

                </nav>

            </div>
        </footer>
    );
};

export default SiteFooter;
