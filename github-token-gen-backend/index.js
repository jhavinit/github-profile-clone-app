...
app.get("/api/tabs/:username", async (req, res) => {
  ...
  try {
    ...
    // 2. stars via repo scanning
    const repoRes = await axios.get(
      `https://api.github.com/users/${username}/repos?per_page=100`,
      { headers: { Authorization: `Bearer ${TOKEN}` } }
    );

    const stars = repoRes.data.reduce((s, r) => s + r.stargazers_count, 0);
    ...
    const packages = pkgRes.data.length;

    // 4. projects (GraphQL v2)
    const query = `
      query($login:String!) {
        user(login: $login) {
    ...
    const projects = projectRes.data.data.user.projectsV2.totalCount;
